...
columns_sbm =['Method','data','Time Step','Mean MAP', 'P@2','P@10','P@100','P@200','P@300','P@500','P@1000','P@EdgeNum' ]
columns_sbmhyp =['Method','batch','epochs','learning rate','embedding','lookback','data','Mean MAP', 'P@2','P@10','P@100','P@200','P@300','P@500','P@1000','P@EdgeNum' ]
columns_sbm1 =['Method','data','Mean MAP', 'P@2','P@10','P@100','P@200','P@300','P@500','P@1000','P@EdgeNum' ]
columns_sbm3 =['Method','Data','Embedding Size','Mean MAP', 'P@2','P@10','P@100','P@200','P@300','P@500','P@1000','P@EdgeNum' ]
columns_sbm2 =['Method','data','lookback','Mean MAP', 'P@2','P@10','P@100','P@200','P@300','P@500','P@1000','P@EdgeNum' ]
...
def main():
    ...
    method_dict   = {'incrementalSVD':'incSVD','rerunSVD':'rerunSVD','optimalSVD':'optimalSVD', 'dynTRIAD':'dynTriad', 'staticAE':'dynGEM','dynAE':'dyngraph2vecAE', 'dynRNN':'dyngraph2vecRNN', 'dynAERNN':'dyngraph2vecAERNN'}
    ...
    if args.testDataType=='sbm_cd' and args.hyperparameter==0 and  args.mean==0 and  args.lookback==0 and args.meanemb==0  and args.table==0:
        ...
        for method in method_s:
            filename=resultdir+'/'+method+'/nm' + str(args.nodemigration)+'_l'+str(args.timelength)+'_emb'+str(args.embeddimension)+'.dlpsumm'
        ...
        if args.figure==1:            
            ...
            g.set_ylim(0.1, 1)
            plt.savefig('./figures/sbm_emb'+str(args.embeddimension)+'_nm'+str(args.nodemigration)+'.pdf',bbox_inches='tight',dpi=300)   
            if args.show==1:
                plt.show()    
        ...
        if args.figure==0:            
            ...
            plt.savefig('./figures/sbm_Ptable_emb'+str(args.embeddimension)+'_nm'+str(args.nodemigration)+'.pdf',bbox_inches='tight',dpi=300) 
            if args.show==1:
                plt.show()   
        ...
        for datatype in testDataType:
            ...
            for method in method_all:
                ...
                filename=resultdir+'/'+method+'/l' + str(length[datatype])+'_emb'+str(args.embeddimension)+'_samples'+str(args.samples)+'.dlpsumm'
        ...
        if args.figure==1:                
            ...
            try:
                ...
                plt.savefig('./figures/'+args.subDataType+'_emb'+str(args.embeddimension)+'_sm'+str(args.samples)+'.pdf',bbox_inches='tight',dpi=300)
                if args.show==1:   
                    plt.show()
            except:
                pdb.set_trace()       
        ...
        if args.figure==0:
            ...
            plt.savefig('./figures/'+args.subDataType+'_Ptable_emb'+str(args.embeddimension)+'_sm'+str(args.samples)+'.pdf',bbox_inches='tight',dpi=300)  
            if args.show==1:
                plt.show()
        ...
        for method in method_all:
            for emb in [64,128, 256]:
                ...
                if args.testDataType=='sbm_cd':
                    filename=resultdir+'/'+method+'/nm' + str(args.nodemigration)+'_l'+str(args.timelength)+'_emb'+str(emb)+'.dlpsumm'
                else:
                    filename=resultdir+'/'+method+'/l' + str(length[args.testDataType])+'_emb'+str(emb)+'_samples'+str(args.samples)+'.dlpsumm'
                with open(filename, 'r') as f:
                    data=f.read().splitlines()[1:]
                    j=0
                    ...
                    df.loc[i]=  [method_dict[method], args.testDataType,emb, dftmp['Mean MAP'], dftmp['P@2'],dftmp['P@10'],dftmp['P@100'],dftmp['P@200'],dftmp['P@300'],dftmp['P@500'],dftmp['P@1000'],dftmp['P@EdgeNum'] ]
                    i+=1 
        ...
        for method in method_all:
            ...
            if args.testDataType=='sbm_cd':
                ...
                filename=resultdir+'/'+method+'/l' + str(length[args.testDataType])+'_emb'+str(args.embeddimension)+'_samples'+str(args.samples)+'.dlpsumm'
            with open(filename, 'r') as f:
                data=f.read().splitlines()[1:]
                j=0
                dftmp= pd.DataFrame(columns=columns_sbm_mean)   
                for t,lines in enumerate(data):
                    ...
                    j+=1           
                dftmp= dftmp.mean(axis=0)              
                df.loc[i]=  [method_dict[method], args.testDataType, dftmp['Mean MAP'], dftmp['P@2'],dftmp['P@10'],dftmp['P@100'],dftmp['P@200'],dftmp['P@300'],dftmp['P@500'],dftmp['P@1000'],dftmp['P@EdgeNum'] ]
                i+=1 
        ...
        if args.figure==1:            
            ...
            g.set_ylim(0.1, 1)
            g.set_ylim(0.0, (df3.max())['Mean MAP']+0.05)
            plt.savefig('./figures/'+args.testDataType+'_emb'+str(args.embeddimension)+'_mean.pdf',bbox_inches='tight',dpi=300)   
            if args.show==1:
                plt.show()    
                    ...
                    for eta in ['0.1', '0.01', '0.001', '0.0001', '1e-5','1e-6']:
                        for emb in range(64,320,64): 
                            for lb in range(1,6, 1):
                                try:
                                    filename=resultdir+'/'+method+'/epoch' + str(epoch)+'_bs'+str(bs)+'_lb'+str(lb)+'_eta'+str(eta)+'_emb'+str(emb)+'.dlpsumm'
                                    # print(filename)
                                    with open(filename, 'r') as f:
                                        data=f.read().splitlines()[1:]
                                        # print(data)
                                        ...
                                        dftmp= dftmp.mean(axis=0) 
                                        # pdb.set_trace()             
                                        df.loc[i]=  [method_dict[method],bs,epoch,eta,emb,lb, args.testDataType, dftmp['Mean MAP'], dftmp['P@2'],dftmp['P@10'],dftmp['P@100'],dftmp['P@200'],dftmp['P@300'],dftmp['P@500'],dftmp['P@1000'],dftmp['P@EdgeNum'] ]
                                        i+=1 
                                except:
        ...
        for method in method_all:
            for datatype in ['hep','AS']:
                for lb in range(1,4):
                    resultdir=resultdir_dict[method]+'/'+datatype
                    if method=='dynAE':
                        filename=resultdir+'/'+method+'/lb_'+ str(lb)+'_l'+ str(length[datatype])+'_emb'+str(args.embeddimension)+'_samples'+str(args.samples)+'.dlpsumm'
                    else:
                        filename=resultdir+'/'+method+'/lb'+ str(lb)+'_l'+ str(length[datatype])+'_emb'+str(args.embeddimension)+'_samples'+str(args.samples)+'.dlpsumm'
                    with open(filename, 'r') as f:
                        data=f.read().splitlines()[1:]
                        j=0
                        dftmp= pd.DataFrame(columns=columns_sbm_mean)   
                        for t,lines in enumerate(data):
                            ...
                            j+=1           
                        dftmp= dftmp.mean(axis=0)              
                        df.loc[i]=  [method_dict[method], datatype,lb, dftmp['Mean MAP'], dftmp['P@2'],dftmp['P@10'],dftmp['P@100'],dftmp['P@200'],dftmp['P@300'],dftmp['P@500'],dftmp['P@1000'],dftmp['P@EdgeNum'] ]
                        i+=1 
        ...
        for datat in datatype:
            ...
            for method in method_all:
                for emb in [64,128, 256]:
                    ...
                    # resultdir=resultdir_dict[method]+'/'+datatype
                    if datat == 'sbm_cd':
                        filename=resultdir+'/'+method+'/nm' + str(20)+'_l'+str(10)+'_emb'+str(emb)+'.dlpsumm'
                    else:
                        filename=resultdir+'/'+method+'/l' + str(50)+'_emb'+str(emb)+'_samples'+str(2000)+'.dlpsumm'
                    with open(filename, 'r') as f:
                        data=f.read().splitlines()[1:]
                        j=0
                        dftmp= pd.DataFrame(columns=columns_sbm_mean)   
                        for t,lines in enumerate(data):
                            ...
                            j+=1           
                        dftmp= dftmp.mean(axis=0)              
                        df.loc[i]=  [method_dict[method], datat,emb, dftmp['Mean MAP'], dftmp['P@2'],dftmp['P@10'],dftmp['P@100'],dftmp['P@200'],dftmp['P@300'],dftmp['P@500'],dftmp['P@1000'],dftmp['P@EdgeNum'] ]
                        i+=1 
