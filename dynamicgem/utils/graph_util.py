...
def sample_graph(di_graph,  n_sampled_nodes=None, node_l=None):
...
def sample_graph_nodes(di_graph, node_l):
    node_num = di_graph.number_of_nodes()
    if  len(node_l) and node_num > len(node_l):
        # node_l = np.random.choice(node_num, n_sampled_nodes, replace=False)
        node_l_inv = {v: k for k, v in enumerate(node_l)}
        sampled_graph = nx.DiGraph()
        sampled_graph.add_nodes_from(range(len(node_l)))
        for st, ed, w in di_graph.edges(data='weight', default=1):
            try:
                v_i = node_l_inv[st]
                v_j = node_l_inv[ed]
...
def sample_graph_withnodes(di_graph, n_sampled_nodes):
    node_num = di_graph.number_of_nodes()
    if  node_num > len(n_sampled_nodes):
        node_l = n_sampled_nodes
        node_l_inv = {v: k for k, v in enumerate(node_l)}
        sampled_graph = nx.DiGraph()
        sampled_graph.add_nodes_from(range(len(n_sampled_nodes)))
        for st, ed, w in di_graph.edges(data='weight', default=1):
            try:
                v_i = node_l_inv[st]
                v_j = node_l_inv[ed]
...
def sample_graph_rw_int(di_graph, n_sampled_nodes=None,
                    random_res_p=0.01, verbose=False):
    t1 = time()
    print('\t\tInt ext sampling. # nodes to sample: %d' % n_sampled_nodes)
    node_l = []
    for n in di_graph.nodes():
        try:
            if di_graph.node[n]["type"] == "external":
                node_l.append(n)
                for in_e in di_graph.in_edges(n):
                    node_l.append(in_e[0])
        except:
            continue
...
def randwalk_DiGraph_to_adj(di_graph, node_frac=0.1,
    ...
    print('Time taken for random walk  on {0} nodes = {1}'.format(n, time() - t0))
    return adj
