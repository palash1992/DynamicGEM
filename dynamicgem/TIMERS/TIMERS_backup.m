function TIMERS(dataFolder)
  ...
  dlmwrite('output/0_U.txt',U_cur,'delimiter',' ','precision',4);
  dlmwrite('output/0_V.txt',V_cur,'delimiter',' ','precision',4);
  ...
  for i = 1:time_slice
     ...
     % A_add = A_add + A_add';   % assume each edge is undirected
      ...
      S_add = getAddedEdge(S_cum,[dataFolder '/' num2str(i)],M); %hange to other functions for other similarities
      ...
      if (Update)
         ...
         % Note that TRIP doesn't ensure smaller loss value
          ...
          dlmwrite(['output/incrementalSVD/' num2str(i) '_U.txt'],U_cur,'delimiter',' ','precision',4);
          dlmwrite(['output/incrementalSVD/' num2str(i) '_V.txt'],V_cur,'delimiter',' ','precision',4);
      ...
      if (Loss_store(i + 1) >= (1 + Theta) * Loss_bound(i + 1))
          ...
          dlmwrite(['output/rerunSVD/' num2str(i) '_U.txt'],U_cur,'delimiter',' ','precision',4);
          dlmwrite(['output/rerunSVD/' num2str(i) '_V.txt'],V_cur,'delimiter',' ','precision',4);
  ...
  for i = 1:time_slice
      ...
      dlmwrite(['output/optimalSVD/' num2str(i) '_U.txt'],temp_U,'delimiter',' ','precision',4);
      dlmwrite(['output/optimalSVD/' num2str(i) '_V.txt'],temp_V,'delimiter',' ','precision',4);
