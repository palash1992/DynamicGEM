function TIMERS_hep(dataFolder,K,Theta)
  %dataFolder='test_data/hep'% directory with the test data consiting off numbers to denote time varying dynamic graph
  time_slice=136%member of time slices to perform the embedding
  ...
  M=hashmapping_AS([dataFolder '/month_136_graph.txt']);
  ...
  dlmwrite('output/hep/0_U.txt',U_cur,'delimiter',' ','precision',4);
  dlmwrite('output/hep/0_V.txt',V_cur,'delimiter',' ','precision',4);
  ...
  for i = 2:time_slice
     ...
     % A_add = A_add + A_add';   % assume each edge is undirected
      ...
      if (Update)
         ...
         % Note that TRIP doesn't ensure smaller loss value
          ...
          dlmwrite(['output/hep/incrementalSVD/' num2str(i) '_U.txt'],U_cur,'delimiter',' ','precision',4);
          dlmwrite(['output/hep/incrementalSVD/' num2str(i) '_V.txt'],V_cur,'delimiter',' ','precision',4);
     ...
     if (Loss_store(i) >= (1 + Theta) * Loss_bound(i))
          ...
          %dlmwrite(['output/hep/rerunSVD/' num2str(i) '_U.txt'],U_cur,'delimiter',' ','precision',4);
          %dlmwrite(['output/hep/rerunSVD/' num2str(i) '_V.txt'],V_cur,'delimiter',' ','precision',4);
  ...
  for i = 2:time_slice
      ...
      dlmwrite(['output/hep/optimalSVD/' num2str(i) '_U.txt'],temp_U,'delimiter',' ','precision',4);
      dlmwrite(['output/hep/optimalSVD/' num2str(i) '_V.txt'],temp_V,'delimiter',' ','precision',4);
