function TIMERS_academic(dataFolder,K,Theta)
 % dataFolder='test_data/academia'% directory with the test data consiting off numbers to denote time varying dynamic graph
  time_slice=35% number of time slices to perform the embedding
  ...
  M=hashmapping([dataFolder '/0']);
  A=parseData([dataFolder '/0'],M); %get the first timeslice data for initialization
  Sim = A;
  N = size(A,1);
  ...
  dlmwrite('output/academia/0_U.txt',U_cur,'delimiter',' ','precision',4);
  dlmwrite('output/academia/0_V.txt',V_cur,'delimiter',' ','precision',4);
  ...
  for i = 1:time_slice
     ...
     % A_add = A_add + A_add';   % assume each edge is undirected
      ...
      S_add =deltaA(S_cum,[dataFolder '/' num2str(i)],M); %change to other functions for other similarities
      ...
      if (Update)
         ...
         % Note that TRIP doesn't ensure smaller loss value
          ...
          dlmwrite(['output/academia/incrementalSVD/' num2str(i) '_U.txt'],U_cur,'delimiter',' ','precision',4);
          dlmwrite(['output/academia/incrementalSVD/' num2str(i) '_V.txt'],V_cur,'delimiter',' ','precision',4);
      ...
      if (Loss_store(i + 1) >= (1 + Theta) * Loss_bound(i + 1))
          ...
          %dlmwrite(['output/academia/rerunSVD/' num2str(i) '_U.txt'],U_cur,'delimiter',' ','precision',4);
          %dlmwrite(['output/academia/rerunSVD/' num2str(i) '_V.txt'],V_cur,'delimiter',' ','precision',4);
  ...
  for i = 1:time_slice
      S_cur =parseData([dataFolder '/' num2str(i)],M);  
      [temp_U,temp_S,temp_V] = svds(S_cur,K);
      temp_U = temp_U * sqrt(temp_S);
      temp_V = temp_V * sqrt(temp_S);
      dlmwrite(['output/academia/optimalSVD/' num2str(i) '_U.txt'],temp_U,'delimiter',' ','precision',4);
      dlmwrite(['output/academia/optimalSVD/' num2str(i) '_V.txt'],temp_V,'delimiter',' ','precision',4);


      Loss_optimal(i + 1) = Obj(S_cur,temp_U,temp_V);
