function TIMERS_sbm_cd(dataFolder,K,Theta)
 ...
 % time_slice=5% number of time slices to perform the embedding
  ...
  dlmwrite('output/sbm_cd/0_U.txt',U_cur,'delimiter',' ','precision',4);
  dlmwrite('output/sbm_cd/0_V.txt',V_cur,'delimiter',' ','precision',4);
     ...
     % A_add = A_add + A_add';   % assume each edge is undirected
      ...
      if (Update)
         ...
         % Note that TRIP doesn't ensure smaller loss value
          ...
          dlmwrite(['output/sbm_cd/incrementalSVD/' num2str(i) '_U.txt'],U_cur,'delimiter',' ','precision',4);
          dlmwrite(['output/sbm_cd/incrementalSVD/' num2str(i) '_V.txt'],V_cur,'delimiter',' ','precision',4);
      ...
      if (Loss_store(i + 1) >= (1 + Theta) * Loss_bound(i + 1))
          ...
          %dlmwrite(['output/sbm_cd/rerunSVD/' num2str(i) '_U.txt'],U_cur,'delimiter',' ','precision',4);
          %dlmwrite(['output/sbm_cd/rerunSVD/' num2str(i) '_V.txt'],V_cur,'delimiter',' ','precision',4);
  ...
  for i = 1:time_slice
      ...
      dlmwrite(['output/sbm_cd/optimalSVD/' num2str(i) '_U.txt'],temp_U,'delimiter',' ','precision',4);
      dlmwrite(['output/sbm_cd/optimalSVD/' num2str(i) '_V.txt'],temp_V,'delimiter',' ','precision',4);
