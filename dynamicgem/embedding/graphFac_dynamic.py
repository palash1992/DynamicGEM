...
class GraphFactorization(DynamicGraphEmbedding):
	...
	def getFVal(self, adj_mtx, X, prev_step_emb = None):
		...
		if prev_step_emb is not None:
			f3 = self._kappa*(np.linalg.norm(X[:prev_step_emb.shape[0], :prev_step_emb.shape[1]] - prev_step_emb)**2)
	...
	def learn_embedding(self, graph, prevEmbed = None):
		...
		for iter_id in range(self._num_iter):
			...
			if not iter_id%100:
				print 'Iter: %d, Del values: %g, del_f1: %g, del_f2: %g, del_f3: %g' % (iter_id, delPhi[0], delPhi1[0], delPhi2[0], delPhi3[0])			
