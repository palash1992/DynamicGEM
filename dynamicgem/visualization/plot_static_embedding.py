...
def plot_static_embedding2D(node_pos, node_colors=None, di_graph=None):
    ...
    if di_graph is None:
        ...
        for i in xrange(node_num):
            pos[i] = node_pos[i, :]
        nx.draw_networkx(di_graph, pos, node_color=node_colors, width=0.1, node_size=100, arrows=False, alpha=0.8, font_size=5)
    return node_pos